...
% Last Modified by GUIDE v2.5 17-May-2017 08:13:23
...
group_of_files=0;
while (num_files{1}~=-1)
    group_of_files=group_of_files+1
    ...
    handles.drta_p.draName=char(fls(num_files{1}));
    handles.drta_p.fullName=char(fls(num_files{1}));
    file_to_cluster = char(fls(num_files{1}));
    %     end
    if ismac
    ...
    if an_error==0
        ...
        cluster_class=[];
        szfls=size(fls);
        offset_chan_file=zeros(szfls(1),4);
        
        noSpikesChFl=zeros(szfls(1),4);
        noSpikesChFlExc=zeros(szfls(1),4);
        f_strings=cell(4,10);
        ...
        for tets=1:4
            ...
            for filNum=1:num_files{1}
                ...
                handles.drta_p.draName=char(fls(filNum));
                cd(directory);
                load([handles.drta_p.draName(1:end-3),'.mat']);
                handles.draq_p=params;
                ...
                handles.drta_p=drta_p;
                handles.drta_p.draName=char(fls(filNum));
                handles.drta_p.fullName=char(fls(filNum));
                handles.par.filename=char(fls(filNum));
            ...
            if nspk>0
                ...
                if size(spikes,1)> handles.par.max_spk
                    naux = floor(handles.par.max_spk/num_files{1});
                    inspk_aux=[];
                    for filNum=1:num_files{1}
                ...
                if print2file==0;
                    ...
                    eval(['print -djpeg40 fig2print_ch' num2str(tets) char(file_to_cluster) '.jpg']);
                end
        ...
        save(outfile, 'num_files', 'fls','directory','sub_directory','offset_chan_file', 'noSpikesChFl','spikesExcluded','all_timestamp',...
